...
Usage() {
    echo "Usage: $0 [--force] --local|--ic"
    echo "--force, if present, must be the first parameter"
    exit 1
}
force=false

# 1) force DB reset
if [ $# -eq 2 -a x$1 == "x--force" ] ; then
    force=true
    shift
fi
echo $force
...
dfx canister call backend  query $network --output json "select * from rbac"  > /dev/null
status=$?
echo status $status, force $force
if [ $status -ne 0 -o $force = true ]  ; then
    echo Creating DB
...
# 4) on a browser, go to the main page (localhost:3000 or grmberg.digitaltechne.ch). Verify:
...
# 5) Only after first login (first rbac row is set):
principal=$(dfx canister call backend $network query --output json "select principal from rbac"  | jq ".Ok"[0][0])

# 6) enable file uploads for the user
auth=$(printf "dfx canister call $network uploads  authorize '(principal %s )'" $principal)
echo $auth
eval "$auth"

# 7) give superpowers to the user
