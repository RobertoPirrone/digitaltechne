...
insert_cartridge() {
    ...
    # sql_cmd="$(printf "insert into cartridge (id, uuid, dna_text, dna_file_asset, inserted_by, lab_name, insert_time, note) select '%s', '%s', dna_text, dna_file_asset, inserted_by, '%s', '%s', '%s' from cartridge where id = 3" $id $uuid "$lab_name" $insert_time $note)"
    sql_cmd="$(printf "insert into cartridge (id, uuid, dna_text, dna_file_asset, inserted_by, lab_name, insert_time, note) select '%s', '%s', '%s', dna_file_asset, inserted_by, '%s', '%s', '%s' from cartridge where id = 3" $id $uuid "$dna_text" "$lab_name" $insert_time $note)"
