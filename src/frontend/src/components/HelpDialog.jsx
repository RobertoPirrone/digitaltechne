...
export function HelpIframe(props) {
    ...
    let manualUrl;
    if (props.fullManual) manualUrl = fullManualUrl;
    else manualUrl = getManualUrl(lang, fullManualUrl, location);
    const iw = 600 - 15 - 6;
    const ih = getBodyHeight() - 15 - 48 - 6;
    return <iframe style={{ border: 0, padding: 4 }} id="helpIframeInDrawer" title="help iframe" width={iw} height={ih} src={manualUrl} />;
}
...
export default function HelpDialog(props) {
    ...
    const handleClickOpen = () => {
        if (props.emptyMode || !props.openHelpInDrawer) setOpen(true);
        else props.openHelpInDrawer();
    };
    const handleClose = () => {
        setOpen(false);
    };
    ...
    return (
        <React.Fragment>
            ...
            <Dialog id="helpDialog" open={open} scroll="paper" maxWidth="lg" fullWidth={true} onClose={handleClose} PaperComponent={PaperComponent} aria-labelledby="draggable-dialog-title">
