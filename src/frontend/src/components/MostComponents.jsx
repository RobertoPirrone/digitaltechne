...
export const MostCheckbox = ({ control, defaultChecked, label, name, onChange, register }) => {
    return <FormControlLabel control={<Checkbox inputRef={register} control={control} name={name} color="primary" onChange={onChange} defaultChecked={defaultChecked} />} label={label} />;
};
...
export const MostButton2 = ({ label, onClick, type = "button", disabled = false, variant = "outlined", color = "secondary", variab, variab_value, className }) => {
...
export const selectValue2obj = (v, options) => {
    for (const i in options) if (options[i].value === v) return options[i];
    return "";
};
...
export const MostSelect = ({
    ...
    return <Controller className={className} as={Select} name={name} value={value} options={options} control={control} required={required} rules={rules} onChange={onChange} defaultValue={defaultValue} placeholder={placeholder} />;
    /*
...
export const MostAutocomplete = ({
    ...
    return (
      ...
      */
                ...
                rules={rules}
            />
            {errors?.[name] && <div className="formFieldError">{t("campo obbligatorio")}</div>}
        </React.Fragment>
    );
};
...
export function CountrySelect({ onChange: ignored, options, name, label, control }) {
    return (
        <Controller
            render={({ onChange, ...props }) => <Autocomplete options={options} getOptionLabel={(option) => option.label} renderInput={(params) => <TextField {...params} label={label} variant="outlined" />} onChange={(e, data) => onChange(data)} {...props} />}
            onChange={([event, data]) => {
                return data;
            }}
            name={name}
...
/**
 * Wrapper unregistred intorno a Autocomplete
 *
 * @function
 * @prop options {array} elementi {label:, id:} del pull down
