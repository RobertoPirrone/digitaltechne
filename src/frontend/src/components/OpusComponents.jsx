...
export const Riservato = (props) => {
  ...
  if (typeof props.reserved === "string") {
    if (props.reserved === "true") reserved = true;
  } else if (typeof props.reserved === "boolean") {
    if (props.reserved) reserved = true;
  } else {
    return <span> props.reserved: unknown type</span>;
  }
  if (reserved) return <span className="riservato"> {t("(riservato)")}</span>;
  return null;
};
...
export const bexplorerLink = (address, transactionHash, returnHref) => {
  ...
  if (address) {
    ...
    if (process.env.REACT_APP_BCID === "3333") href = "/address/";
    else href = "/account/";
    href = process.env.REACT_APP_BEXPLORER + href + address;
  } else {
    if (!transactionHash) return null;
    transactionHash = transactionHash.toLowerCase();
    if (process.env.REACT_APP_BCID === "3333") href = "/tx/";
    else href = "/transaction/";
...
export const BexplorerLink = (props) => {
  ...
  if (!href) return null;
  return (
    <a href={href} target="blockExplorer" rel="noreferrer">
      Block Explorer
    </a>
  );
};
